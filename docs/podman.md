...
| `no interpreter found for Python==3.N.*` during `uv sync` | `project.requires-python` in that project's `pyproject.toml` excludes the image's interpreter. Both projects should say `>=3.10`; a narrower range such as `>=3.11, <3.12` produces exactly this | fix `requires-python`. `UV_PYTHON` does NOT override it -- it turns the error into "The requested interpreter resolved to Python 3.12.x, which is incompatible with the project's Python requirement" |
| Anything else odd in a hand-copied tree | the working copy has drifted from the repo | `diff` the manifests against a clean checkout -- `pyproject.toml`, `uv.lock`, `Dockerfile`, `.dockerignore` |
