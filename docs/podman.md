...
| `no interpreter found for Python==3.N.*` during `uv sync` | something in the project asks for an interpreter the image lacks -- a stray `.python-version`, or a `.venv` built elsewhere on another version that uv tries to match. Neither lockfile pins a version | already handled: the Dockerfile sets `UV_PYTHON=3.12`, which overrides both. Also check `ls -a agent-client/` for a `.python-version` that should not be there |
